...
function fillPlaceholders() {
  for (let i = 0; i < document.placeholders.length; i++) {
    ...
    if (isReservedClassName(phObj.name)) {
      throw new BuildException(
        exceptions.IllegalName,
        `The name '${phObj.name}' is reserved keyword and can not be used as enum class name.`);
    }
...
function fillHashsetinClasses() {
  ...
  for (let i = 0; i < document.hashsets.length; i++) {
    ...
    if (isReservedClassName(hashset.entity)) {
      throw new BuildException(
        exceptions.IllegalName,
        `The name '${hashset.entity}' is reserved keyword and can not be used as entity class name.`);
    }

    if (!(hashset.entity in jdlObject.entities)) {
      console.log(`Hashset Entry found for '${hashset.entity}', but entity definition is missing in JDL file...`);
      throw new BuildException(
        exceptions.IllegalName,
        `The name '${hashset.entity}' doesn't match with the entity class names available.`);
    }

    if (!(hashset.target.entity in jdlObject.entities)) {
      console.log(`Hashset Entry found for '${hashset.target.entity}', but entity definition is missing in JDL file...`);
      throw new BuildException(
        exceptions.IllegalName,
        `The name '${hashset.target.entity}' doesn't match with the entity class names available.`);
    }
...
function fillScreens() {
  ...
  for (let i = 0; i < document.screens.entity.length; i++) {
    ...
    if (isReservedClassName(screen.name)) {
      throw new BuildException(
        exceptions.IllegalName,
        `The name '${screen.name}' is reserved keyword and can not be used as entity class name.`);
    }
...
function fillCombolist() {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (isReservedClassName(clItem.entity)) {
      throw new BuildException(
        exceptions.IllegalName,
        `The name '${clItem.entity}' is reserved keyword and can not be used as entity class name.`);
    }

    if (!(clItem.entity in jdlObject.entities)) {
      console.log(`Combolist Entry found for '${clItem.entity}', but entity definition is missing in JDL file...`);
      throw new BuildException(
        exceptions.IllegalName,
        `The name '${clItem.entity}' doesn't match with the entity class names available.`);
    }
...
function fillTemplates() {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (!(clItem.entity in jdlObject.entities)) {
      console.log(`Template Entry found for '${clItem.entity}', but entity definition is missing in JDL file...`);
      throw new BuildException(
        exceptions.IllegalName,
        `The name '${clItem.entity}' doesn't match with the entity class names available.`);
    }
