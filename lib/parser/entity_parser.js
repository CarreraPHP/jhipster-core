...
const dateFormatForLiquibase = require("../utils/format_utils")
...
function checkNoSQLModeling(passedJdlObject, passedDatabaseType) {
  if (
    passedJdlObject.relationships.size !== 0 &&
    !DatabaseTypes.isSql(passedDatabaseType)
  ) {
    throw new BuildException(
      exceptions.NoSQLModeling,
      "NoSQL entities don't have relationships."
    );
  }
}
...
function initializeEntities() {
  for (
    let i = 0, entityNames = Object.keys(jdlObject.entities);
    ...
    if (entityName.toLowerCase() === USER.toLowerCase()) {
      winston.warn(
        "Warning:  An Entity name 'User' was used: 'User' is an" +
      ...
      entities[entityName] = {
        // todo: make a builder/factory instead!
        fluentMethods: true,
        relationships: [],
        fields: [],
    ...
    if (option.entityNames.size() === 1 && option.entityNames.has("*")) {
      option.entityNames = _.filter(
        ...
        entityName =>
          ...
          entityName.toLowerCase() !== USER.toLowerCase()
      );
    }
...
function setEntityNamesOptions(option) {
  option.entityNames.forEach(entityName => {
    // console.log(entityName, option.name, entityName in entities, option.value);
    if (entityName in entities) {
      switch (option.name) {
        ...
        case UnaryOptions.FILTER:
          ...
          break;
        case BinaryOptions.APPROVAL:
          entities[entityName].hasMakerCheckerApproval = true;
          break;
        case BinaryOptions.AUTO_APPROVAL:
          entities[entityName].hasMakerCheckerAutoApproval = true;
          break;
        case BinaryOptions.SEARCH_ENGINE:
          if (option.value === "advancesearch") {
            entities[entityName].enableAdvanceSearch = true;
          } else if (option.value === "columnsearch") {
            entities[entityName].enableColumnSearch = true;
          } else {
            if (option.value === "elasticsearch") {
              entities[entityName].enableElasticSearch = true;
            }
            entities[entityName][option.name] = option.value;
          }
          break;
        case BinaryOptions.FEATURES:
          if (option.value === "import") {
            entities[entityName].enableImport = true;
          } else if (option.value === "export") {
            entities[entityName].enableExport = true;
          } else if (option.value === "bulkapproval") {
            entities[entityName].enableBulkApproval = true;
          } else if (option.value === "bulksubmit") {
            entities[entityName].enableBulkSubmit = true;
          } else if (option.value === "viewhistory") {
            entities[entityName].enableViewHistory = true;
          } else {
            entities[entityName][
              `enable${_.upperFirst(_.camelCase(option.name))}`
            ] = option.value;
          }
          break;
        default:
...
function fillEntities() {
  for (
    ...
    if (entityName.toLowerCase() !== USER.toLowerCase()) {
      ...
      setMultilingualFieldsOfEntity(entityName);
      setRelationshipOfEntity(entityName);
      setHashSetOfEntity(entityName);
      setComboList(entityName);
      setTemplate(entityName);
      setPlaceholder(entityName);
    }
  }
}

function setPlaceholder(entityName) {
  entities[entityName].placeAt = jdlObject.entities[entityName].placeAt;
  entities[entityName].placeholders = jdlObject.placeholders;
}

function setMultilingualFieldsOfEntity(entityName) {
  for (
    let i = 0,
      fieldNames = Object.keys(
        jdlObject.entities[entityName].multilingualFields
      );
    i < fieldNames.length;
    i++
  ) {
    ...
    const jdlField =
      jdlObject.entities[entityName].multilingualFields[fieldName];
    const fieldData = {
    ...
    fieldData.fieldLabel = jdlField.label ? jdlField.label : "";
    fieldData.isCommonModule = !!jdlField.isCommonModule;
    fieldData.commonModuleName = jdlField.commonModuleName;
    fieldData.precision = jdlField.precision;
    ...
    entities[entityName].hasMultiLingualFields =
      jdlObject.entities[entityName].hasMultiLingualFields;
    entities[entityName].multilingualFields.push(fieldData);
  }
}

function setFieldsOfEntity(entityName) {
  for (
    let i = 0, fieldNames = Object.keys(jdlObject.entities[entityName].fields);
    i < fieldNames.length;
    i++
  ) {
    const fieldName = fieldNames[i];
...
function setHashSetOfEntity(entityName) {
  if (entityName in entities && entityName in jdlObject.entities) {
    entities[entityName].hashsets = jdlObject.entities[entityName].hashsets;
  }
}

function setComboList(entityName) {
  if (entityName in entities && entityName in jdlObject.entities) {
    entities[entityName].combolist = jdlObject.entities[entityName].combolist;
  }
}

function setTemplate(entityName) {
  if (entityName in entities && entityName in jdlObject.entities) {
    entities[entityName].template = jdlObject.entities[entityName].template;
    entities[entityName].templateFileName =
      jdlObject.entities[entityName].templateFileName;
    entities[entityName].templateDomainGroup =
      jdlObject.entities[entityName].templateDomainGroup;
    entities[entityName].templateUIProjectName =
      jdlObject.entities[entityName].templateUIProjectName;
    entities[entityName].templateIsChild =
      jdlObject.entities[entityName].templateIsChild;
  }
}
...
function setValidationsOfField(jdlField, fieldData) {
  ...
  for (
    let i = 0, validationNames = Object.keys(jdlField.validations);
    i < validationNames.length;
    i++
  ) {
...
function setRelationshipOfEntity(entityName) {
  const relatedRelationships = getRelatedRelationships(
    entityName,
  ...
  for (let i = 0; i < relatedRelationships.from.length; i++) {
    ...
    if (relatedRelationship.type === RelationshipTypes.ONE_TO_ONE) {
      ...
      relationship.otherEntityRelationshipName = _.lowerFirst(
        relatedRelationship.injectedFieldInTo || relatedRelationship.from.name
      );
      relationship.actualFields = relatedRelationship.injectActualFields;
      ...
      relationship.relationshipName = camelCase(
        splitField.relationshipName || relatedRelationship.to.name
      );
      relationship.otherEntityName = camelCase(relatedRelationship.to.name);
      relationship.otherEntityRelationshipName = _.lowerFirst(
        otherSplitField.relationshipName
      );
      relationship.actualFields = relatedRelationship.injectActualFields;
      if (!relatedRelationship.injectedFieldInTo) {
        relationship.otherEntityRelationshipName = _.lowerFirst(
          relatedRelationship.from.name
        );
        otherSplitField = extractField(relatedRelationship.injectedFieldInTo);
        ...
        otherSideRelationship.actualFields =
          relatedRelationship.injectActualFields;
        entities[relatedRelationship.to.name].relationships.push(
          otherSideRelationship
        );
      }
    } else if (
      relatedRelationship.type === RelationshipTypes.MANY_TO_ONE &&
      relatedRelationship.injectedFieldInFrom
    ) {
      splitField = extractField(relatedRelationship.injectedFieldInFrom);
      relationship.relationshipName = camelCase(splitField.relationshipName);
      relationship.otherEntityName = camelCase(relatedRelationship.to.name);
  ...
  for (let i = 0; i < relatedRelationships.to.length; i++) {
    ...
    if (relatedRelationship.type === RelationshipTypes.ONE_TO_ONE) {
      ...
      relatedRelationship.injectedFieldInTo =
        relatedRelationship.injectedFieldInTo ||
        _.lowerFirst(relatedRelationship.from);
      splitField = extractField(relatedRelationship.injectedFieldInTo);
      relationship.relationshipName = camelCase(
        splitField.relationshipName || relatedRelationship.from.name
      );
      relationship.otherEntityName = camelCase(relatedRelationship.from.name);
      relationship.otherEntityField = _.lowerFirst(splitField.otherEntityField);
      relationship.actualFields = relatedRelationship.injectActualFields;
