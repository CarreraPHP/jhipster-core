    ...
    if (entityName in entities) {
      ...
      case BinaryOptions.FEATURES:
        if (option.value === 'import') {
          ...
          entities[entityName][`enable${_.upperFirst(_.camelCase(option.name))}`] = option.value;
        }
        break;
      default:
        entities[entityName][option.name] = option.value;
...
function setMultilingualFieldsOfEntity(entityName) {
  for (let i = 0, fieldNames = Object.keys(jdlObject.entities[entityName].multilingualFields); i < fieldNames.length; i++) {
    const fieldName = fieldNames[i];
    const jdlField = jdlObject.entities[entityName].multilingualFields[fieldName];
    const fieldData = {
    ...
    entities[entityName].hasMultiLingualFields = jdlObject.entities[entityName].hasMultiLingualFields;
  ...
  for (let i = 0; i < relatedRelationships.from.length; i++) {
    ...
    if (relatedRelationship.type === RelationshipTypes.ONE_TO_ONE) {
      ...
      if (!relatedRelationship.injectedFieldInTo) {
        ...
        otherSideRelationship.actualFields = relatedRelationship.injectActualFields;
